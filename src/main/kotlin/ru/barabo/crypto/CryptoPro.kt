...
import ru.barabo.onewin.xml.request.XmlValidator
...
object CryptoPro {
    ...
    private val certificateData: CertificateData = initCertByBuild()
    ...
    private fun signWithTimeCommand(sourceFile: String, signedFile: String) =
        "$CRYPTO_FULL_PATH -sfsign -sign -in $sourceFile -out $signedFile -my ${certificateData.my} -add -addsigtime"
    ...
    fun unsign(signed: File, unSigned: File): File {
        ...
        if(!signed.exists()) {
            throw IOException("signed file ${signed.absolutePath} is not exists")
        }
    ...
    @Throws(IOException::class)
    fun signWithTime(source: File, signed: File): File {

        val cmd = signWithTimeCommand(source.absolutePath, signed.absolutePath)
...
fun unsignXml(signFile: File, unsignFile: File) {
    if(!unsignFile.parentFile.exists()) {
        unsignFile.parentFile.mkdirs()
    }

    CryptoPro.unsign(signFile, unsignFile)

    if(unsignFile.exists()) {
        signFile.parentFile.deleteFolder()
    }
}

fun signXml(xmlText: String, xsdSchemaPath: String): File {

    val tempFolder = Cmd.tempFolder("sign")

    val tempFileName = "${Date().time}.xml"

    val source = File("${tempFolder.absolutePath}/$tempFileName")

    source.writeText(xmlText)

    XmlValidator.validate(source, xsdSchemaPath)

    val sign = File("${tempFolder.absolutePath}/${source.name}.sig")

    CryptoPro.signWithTime(source, sign)
...
const val XSD_SCHEMA_REQUEST = "/xsd/qcb_request.xsd"
